...
def adicionar_ou_atualizar_usuario(username, creditos, data_ultima_compra):
    """
    adicionar_ou_atualizar_usuario(username_of_registered_user, 30, datetime.now().strftime('%Y-%m-%d'))
...
def importar_planilha_entregas(df):
    return pd.read_excel(df, dtype={'Código': str, 'Endereço': str}, skiprows=1)


@st.cache_data
def teste_planilha(df, limite_carga):
    required_columns = ['Código', 'Endereço', 'Carga']

    if not all(col in df.columns for col in required_columns):
        return 'Nomes das colunas diferem do modelo'

    try:
        df['Carga'] = pd.to_numeric(df['Carga'], errors='raise')
    except ValueError:
        return 'Os dados da coluna Carga não são todos numéricos'

    if df['Código'].duplicated().any():
        return 'Existem códigos duplicados na planilha'

    if len(df['Carga']) > 100:
        return 'Quantidade de entregas excede o limite de 100 entregas por vez'

    if df.isnull().any().any():
        return 'Existem valores faltantes na planilha'

    if df['Carga'].min() <= 0:
        return 'Há entregas com Carga menor que zero'

    if df['Carga'].max() > limite_carga:
        return 'Há entregas com carga maior que o limite estabelecido'
...
def checar_se_algum_endereco_nao_foi_obtido(df):
    ...
    if not codigos_sem_endereco.empty:
        st.warning('Alguns endereços não foram obtidos:')
...
def disponibilizar_download_pasta_como_zip(pasta_path, col8, nome_arquivo_zip='arquivos.zip'):
    ...
    col8.header('Baixar PDFs com rotas', divider='blue')
...
def plot_map(df, cluster_selecionado, tamanho_ponto):
    ...
    data = [{
        ...
        "tooltip": "Local de Partida" if idx == 0 else f"Parada {idx}"
...
def pagina_principal():
    ...
    # Endereço de partida
    st.header('Endereço de partida', divider='blue')
    col1, col2, col3 = st.columns([3, 1, 3])
    coordenadas_partida = None
    with col2:
        add_vertical_space(2)
    endereco_partida = col1.text_input('Digite o endereço de partida:')
    if col2.button('Validar'):
        if endereco_partida:
    ...
    if coordenadas_partida:
        with col3:
            add_vertical_space(1)
            st.success('Endereço validado')

    # Parametros da simulação
    add_vertical_space(2)
    st.header('Definição dos parametros', divider='blue')
    col4, col5, col6 = st.columns(3)
    with col4:
        st.subheader('Entregadores')
        numero_de_entregadores = st.number_input('Quantidade de entregadores', step=1, key='numero_de_entregadores', value=1)
        if numero_de_entregadores <= 0:
            st.error('valor invalido')
    with col5:
        st.subheader('Carga máxima')
    ...
    with col6:
        ...
        otimizador = st.selectbox("Tipo de otimização", ["carga", "entregas"], key='otimizador')

    # Planilha de cadastro de entregas
    add_vertical_space(2)
    st.header('Enviar planilha de entregas', divider='blue')

    col10, col11 = st.columns(2)
    my_file_path = "BackEnd/Templete de cadastro de entregas.xlsx"
    col11.markdown(f'<div style="font-size: {20}px">{"Baixar planilha modelo"}</div>', unsafe_allow_html=True)
    col11.markdown(get_binary_file_downloader_html(my_file_path, 'Clique aqui', 'Templete de cadastro de entregas.xlsx'), unsafe_allow_html=True)
    planilha_de_entregada = col10.file_uploader('Upload da planilha de entregas', accept_multiple_files=False, label_visibility='collapsed')
        ...
        if status_planilha == 'sucesso':
            ...
            if df_entregas is None:
                ...
                if checar_se_algum_endereco_nao_foi_obtido(df_entregas):
                    ...
                    col7.header('Realizar calculo de rotas', divider='blue')

                    if 'df_final_rotas' not in st.session_state:
                        st.session_state.df_final_rotas = None

                    if (col7.button('Calcular rotas')) & (len(df_entregas['Carga']) <= consultar_usuario(st.session_state["auth"])[0]):
                        ...
                        gerar_pdf_para_cluster(df_final, diretorio, 'br')
                        ...
                        if len(df_entregas['Carga']) > consultar_usuario(st.session_state["auth"])[0]:
                            col7.error('Você não possui créditos suficientes para realizar o cálculo de rotas')

                    if st.session_state.df_final_rotas is not None:
                        # Sempre renderiza os widgets nos placeholders definidos
                        tamanho_ponto = placeholder_slider.slider('Tamanho do ponto', min_value=50, max_value=1000,
                                                                  value=50, step=50, key='tamanho_ponto')
                        cluster_selecionado = placeholder_selectbox.selectbox('Selecione uma rota',
